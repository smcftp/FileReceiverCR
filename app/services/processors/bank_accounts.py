...
def convert_currency(balance_currency: str, amount: float) -> float:
    """
    ...
    Returns:
        float: Конвертированная сумма в BYN
    """
    # Словарь с курсами валют к BLR
    exchange_rates = {
        "USD": 3.07,  # Пример курса USD к BLR
        "EUR": 3.5,  # Пример курса EUR к BLR
        "RUR": 0.037,  # Пример курса RUB к BLR
        "KZT": 0.0059,  # Пример курса KZT к BLR
        "UZS": 0.0002,  # Пример курса UZS к BL
        # Добавьте другие валюты и их курсы здесь
    }
    
    rate = exchange_rates.get(balance_currency)
    if rate is not None:
        return amount * rate
    else:
        ...
        for idx, record in enumerate(records):
            ...
            try:
                ...
                if balance_raw == balance_byn_raw and balance_currency != "BLR":
                    ...
                    balance_byn_raw = convert_currency(balance_currency, balance_raw)
                    logger.info(f"Конвертированная сумма в BYN: {balance_byn}")
                else:
                    balance_byn_raw = balance_byn_raw if balance_byn_raw is not None else balance_raw  # Используем balance_byn_raw или balance_raw
                ...
                if balance_byn_raw is not None:
                    ...
                    balance_byn = balance  # Если нет balance_byn, используем balance
