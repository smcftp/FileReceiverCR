...
from sqlalchemy.dialects.postgresql import JSONB, UUID, MONEY
...
class Loan(Base):
    ...
    loan_currency: Mapped[str] = mapped_column(String(3), nullable=False, comment="Валюта займа")
    
    duty_money: Mapped[Decimal] = mapped_column(MONEY(), nullable=False, comment="Долг (в валюте money)")
    duty_byn_money: Mapped[Decimal] = mapped_column(MONEY(), nullable=False, comment="Долг в BYN (в валюте money)")
    
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="active", comment="Статус займа (активный/закрытый)")
